...
let
  ...
  supportedGpus = builtins.filter isSupported cudaModules.config.gpus;
  ...
  jetsonTargets = lists.intersectLists jetsonComputeCapabilities cudaModules.config.cudaCapabilities;
...
// formatCapabilities {
  cudaCapabilities =
    if cudaModules.config.cudaCapabilities == [ ] then
      defaultCapabilities
    else
      cudaModules.config.cudaCapabilities;
  enableForwardCompat = cudaModules.config.cudaForwardCompat;
}
